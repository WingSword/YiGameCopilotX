...
class AndroidLANClient : LANClient {
    ...
    override suspend fun connect(hostAddress: String, port: Int, playerName: String): Boolean {
        ...
        return withContext(Dispatchers.IO) {
            try {
                client = HttpClient {
                    install(WebSockets) {
                        pingInterval = HEARTBEAT_INTERVAL.milliseconds
                    }
                }
                ...
                GameLogger.info("已连接到主机: $hostAddress:$port")
                true
            } catch (e: Exception) {
                        ...
                        try {
                            ...
                            when (message.type) {
                                LANMessageType.JOIN_RESPONSE -> {
                                    ...
                                    GameLogger.info("加入成功: ${_currentPlayer?.name}")
                                }
    ...
    override suspend fun disconnect() {
        ...
        try {
            ...
            GameLogger.error("关闭会话失败", e)
        }
        
        try {
            client?.close()
        } catch (e: Exception) {
            GameLogger.error("关闭客户端失败", e)
        }
